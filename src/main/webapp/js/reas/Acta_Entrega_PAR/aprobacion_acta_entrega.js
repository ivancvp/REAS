...
function aprobacion_acta_entrega(identificador,id_actividad,tipo_proceso,tipo_actividad,actividad_padre,asignado_a,observacion,modo){
...
'<h1>Aprobación Cargue de Acta de entrega de la ocupación</h1>'+
'<p style="color:#5F6A6A; font-weight: bold ">Dirección de Reasentamientos</p>'+
 '<hr /> '+
'<p>Cargue de Acta de Entrega de la Ocupación por:</p>'+    
...
'<br>';
 ...
 function logica_aprobacion_acta_entrega(datos,modo){
        ...
        if(modo===2){
            ...
            var dat1=consulta_acta_entrega_aprobado(identificador);
    ...
    $( "#sel_aprobacion" ).change(function() {        
     ...
     $('#aprob_estudio').click(function () {
        ...
        envio_de_notificacion(identificador,1,8,9,creador,asignado_a,estado,observacion_inicial,observacion_final);       
        quitar_tarea_lider(id_actividad);

        insertar_aprobacion_acta_entrega(identificador,$('#nom_crea').val(),$('#aprobo_est').val(),$('#sel_aprobacion').val(),$('#obs_regreso').val());
        ...
        correo(creador,asignado_a,"Aprobación del Cargue del Acta de Entrega de la Ocupación",msg,tipo_proceso);
        
        if($('#sel_aprobacion').val()==="true"){
           actualizar_estado_acta_entrega(identificador);
        }
